...
# Ops omit `page` on purpose: the server must default to the first page.
set -e
cd "$(dirname "$0")/.."
WS=$(mktemp -d)/ws
...
test "$(S | J 'j.pages[0].sections.map(x=>x.slug).join(",")')" = "nav,features,hero"
...
test "$(S | J 'j.pages[0].sections.find(x=>x.slug==="hero").active')" = "1"
...
test "$(S | J 'j.pages[0].sections.length')" = "2"
test -f "$WS/site/sections/index/features/take-1.html"
