...
function reporterFor(slug) {
  return `<script>(function(){var S=${JSON.stringify(slug)},last=-1,sched=false;function H(){return Math.max(document.documentElement.scrollHeight,document.body?document.body.scrollHeight:0,1);}function post(){sched=false;var h=H();if(h===last)return;last=h;try{parent.postMessage({type:'rb-h',slug:S,h:h},'*');}catch(e){}}function R(){if(sched)return;sched=true;(window.requestAnimationFrame||function(f){setTimeout(f,16);})(post);}window.addEventListener('load',R);if(document.readyState==='complete')R();try{if(document.fonts&&document.fonts.ready)document.fonts.ready.then(R);}catch(e){}try{var ro=new ResizeObserver(R);ro.observe(document.documentElement);if(document.body)ro.observe(document.body);}catch(e){}[50,200,500,1000].forEach(function(t){setTimeout(R,t);});})();</script>`;
}
